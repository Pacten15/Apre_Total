...
#* Partition Data
kNN_X, kNN_y = df_scaled.iloc[:, 0:num_columns-1], df_scaled["class"]
NB_X, NB_y = df.iloc[:, 0:num_columns-1], df["class"]
...
kNN_acc_folds = []
NB_acc_folds = []
for train_k, test_k in folds.split(kNN_X, kNN_y):
    X_train, X_test = kNN_X.iloc[train_k], kNN_X.iloc[test_k]
    y_train, y_test = kNN_y.iloc[train_k], kNN_y.iloc[test_k]
...
for train_k, test_k in folds.split(NB_X, NB_y):
    X_train, X_test = NB_X.iloc[train_k], NB_X.iloc[test_k]
    y_train, y_test = NB_y.iloc[train_k], NB_y.iloc[test_k]
...
# para um limite de confiança alpha = 0.05, tem-se que kNN > NB, uma vez que
# o respetivo p-value é menor que alpha, i.e., 0.00024 < p-value
